...
class Demo:
    ...
    # Default assignment target position for Part 2 Q7
    default_place_x = 0.45
    ...
    def set_speed_profile(self, profile: str) -> None:
        ...
        if profile == "fast":
            self.max_speed_xy = 0.55
            self.max_speed_z = 0.45
            self.min_move_dur = 0.12
            self.max_move_dur = 1.60

        elif profile == "slow":
            self.max_speed_xy = 0.12
            self.max_speed_z = 0.10
            self.min_move_dur = 0.45
            self.max_move_dur = 4.00

        else:
            self.max_speed_xy = 0.30
            self.max_speed_z = 0.25
            self.min_move_dur = 0.25
            self.max_move_dur = 2.50
    ...
    def set_object_xy(self, obj_name: str, x: float, y: float) -> None:
        """
        Move an object to a new XY position in simulation.
        Works when the object body has a free joint.
        """
        obj = self._require_body(obj_name)
        ...
        if jadr < 0:
            raise RuntimeError(f"Body '{obj}' has no joint to reposition.")

        jtype = int(self.model.jnt_type[jadr])
        if jtype != mujoco.mjtJoint.mjJNT_FREE:
            raise RuntimeError(f"Body '{obj}' is not using a free joint, so XY reset is not supported here.")

        qadr = int(self.model.jnt_qposadr[jadr])

        # keep current z and quaternion, only change x/y
        self.data.qpos[qadr + 0] = float(x)
        self.data.qpos[qadr + 1] = float(y)

        mujoco.mj_forward(self.model, self.data)

    def task_success_xy(self, obj_name: str, target_x: float, target_y: float,
                        xy_tol: float = 0.05, z_min: float = 0.0, z_max: float = 0.10) -> bool:
        """
        Success condition:
        - object is close to target XY
        - object is on the table area (reasonable z)
        - robot is no longer holding it
        """
        obj = self._require_body(obj_name)
        b = self.data.body(obj)

        dxy = float(np.linalg.norm(b.xpos[:2] - np.array([target_x, target_y], dtype=float)))
        z = float(b.xpos[2])

        placed_near_target = dxy <= xy_tol
        on_table = (z_min <= z <= z_max)
        released = (self.held_obj is None)

        return bool(placed_near_target and on_table and released)

    def timed_pick_place_xy(self, obj: str, x: float, y: float) -> tuple[int, float]:
        """
        Run one timed trial.
        Returns:
            success as 1/0
        ...
        success = 1 if self.task_success_xy(obj, x, y) else 0
        elapsed = float(t1 - t0)
        return success, elapsed

    def run_trials(self, obj: str, target_x: float, target_y: float,
                   positions: list[tuple[float, float]],
                   csv_path: str = "trial_results.csv") -> list[tuple[int, int, float, float, float]]:
        """
        Run multiple trials with different object positions.
        CSV columns:
        trial,success,time,obj_x,obj_y
        """
        results: list[tuple[int, int, float, float, float]] = []

        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["trial", "success", "time", "obj_x", "obj_y"])

            for i, (ox, oy) in enumerate(positions, start=1):
                if self.stop_flag.is_set():
                    break
                ...
                self.set_object_xy(obj, ox, oy)
                self.wait(0.20)

                success, elapsed = self.timed_pick_place_xy(obj, target_x, target_y)

                writer.writerow([i, success, round(elapsed, 3), ox, oy])
                results.append((i, success, round(elapsed, 3), ox, oy))

                print(f"trial={i}, success={success}, time={elapsed:.3f}, obj_x={ox:.3f}, obj_y={oy:.3f}")
...
def launch_gui(demo: Demo) -> None:
    ...
    place_x = tk.StringVar(value=str(demo.default_place_x))
    place_y = tk.StringVar(value=str(demo.default_place_y))
    ...
    speed_profile = tk.StringVar(value="normal")
    ttk.Label(root, text="Speed profile:").grid(row=5, column=0, sticky="e", padx=6)
    ttk.Combobox(root, width=12, textvariable=speed_profile, values=["slow", "normal", "fast"], state="readonly").grid(row=5, column=1, padx=4, sticky="w")

    def run_pickplace_now():
        try:
            demo.set_speed_profile(speed_profile.get())
    ...
    ttk.Button(root, text="Run Pick & Place XY", command=run_pickplace_now).grid(row=6, column=0, padx=8, pady=12, sticky="w")
    ttk.Label(root, text="Viewer console NL: run with --viewer-console").grid(row=7, column=0, columnspan=4, padx=8, sticky="w")
    ttk.Label(root, text="Voice NL: run with --voice-terminal").grid(row=8, column=0, columnspan=4, padx=8, sticky="w")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--speed-profile", choices=["slow", "normal", "fast"], default="normal",
                        help="Assignment speed comparison: slow, normal, or fast.")
    parser.add_argument("--default-place-x", type=float, default=0.45,
                        help="Default GUI target X for assignment Q7.")
    parser.add_argument("--default-place-y", type=float, default=0.10,
                        help="Default GUI target Y for assignment Q7.")

    parser.add_argument("--run-trials", action="store_true",
                        help="Run multiple timed trials and save a results table.")
    parser.add_argument("--csv", type=str, default="trial_results.csv",
                        help="CSV output file for trial results.")
    ...
    demo.default_place_x = float(args.default_place_x)
    demo.default_place_y = float(args.default_place_y)
    demo.set_speed_profile(args.speed_profile)
        ...
        def run_sequence():
            ...
            if args.run_trials:
                if args.x is None or args.y is None:
                    raise ValueError("Trial mode requires --x and --y for the target placement position.")
